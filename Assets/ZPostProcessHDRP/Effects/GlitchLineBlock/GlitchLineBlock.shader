Shader "Hidden/Shader/GlitchLineBlock"
{
    HLSLINCLUDE
	...
	float4 Frag_Horizontal(Varyings i): SV_Target
	{
		...
		float4 blockLineColor = LOAD_TEXTURE2D_X(_InputTexture,  abs(uv_blockLine)*_ScreenSize.xy);
		...
		// [5] 与源场景图进行混合
		float4 sceneColor = LOAD_TEXTURE2D_X(_InputTexture,  i.texcoord*_ScreenSize.xy);
